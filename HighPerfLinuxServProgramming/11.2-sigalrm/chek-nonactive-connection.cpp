    ...
    while (!stop_server)
    {
        ...
        for (i = 0; i < number; i++)
        {
            ...
            if (fd == listenfd)
            {
                ...
                //创建定时器，设置期回调函数与超时时间，绑定定时器与用户数据，最后将定时器添加到链表timer_lst中
            ...
            else if ((fd == pipefd[0]) && (events[i].events & EPOLLIN)) //处理信号
            {
                int sig;
                char signals[1024];
                ...
                if (ret == -1)
                {
                    printf("recv signal info failed");
                    continue;
                }
                else if (ret == 0)
                {
            ...
            else if (events[i].events & EPOLLIN) //处理客户连接上接收到的数据
            {
                ...
                {
                    //如果某个客户连接上有数据可读，则我们调整该连接对应的定时器，以延迟该连接被关闭的时间
                    if (timer)
                    {
                        time_t cur = time(NULL);
                        timer->expire = cur + 3 * TIMESLOT;
        ...
        //最后处理定时时间，因为I/O事件有更高的优先级。这会导致定时任务不能精确地按照预期的时间执行
        if (timeout)
        {
            timer_handler();
            timeout = false;
